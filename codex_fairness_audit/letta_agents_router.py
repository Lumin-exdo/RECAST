...
async def capture_messages(
    ...
    # Captured external conversations must participate in the agent's ordered
    # message history. Persisting the rows alone leaves them detached from the
    # in-context message list and makes MemGPT's conversation_search unable to
    # retrieve them reliably.
    await server.agent_manager.append_to_in_context_messages_async(
        messages=messages_to_persist, agent_id=agent_id, actor=actor
    )
    response_messages = messages_to_persist
