#!/usr/bin/env python3
"""
Ablation A: w/o E2E Answer Generation.

Loads G2's final write-phase state (profile_snapshot at the end of all
sessions, from runs/d215489/e2ev3_400/{idx:04d}/trace.json) for all 400
samples, then runs the query phase with query_hypothesis expansion +
retrieval UNCHANGED — only the final answer-generation prompt is swapped
from the structured 4-step CoT (E2E_ANSWER_PROMPT) to a naive one
(NAIVE_ANSWER_PROMPT, same inputs, no explicit assumption-check steps).

Per-sample checkpointing: writes {idx:04d}/answer.json as each sample
completes; reruns skip samples that already have one.

Run from /mnt/laq:
  python -m RECAST.scripts.query_only_naive_answer --run-name a_naive_answer --workers 8
"""
...
def parse_args():
    ...
    p.add_argument("--run-name", default="a_naive_answer")
...
def main():
    ...
    if args.uids:
        ...
        records = list(enumerate(all_records))
        records = [(r, i) for i, r in records]

    run_dir = Path(args.output_dir).resolve() if args.output_dir else DEFAULT_RUNS_ROOT / "bugfix" / args.run_name
    run_dir.mkdir(parents=True, exist_ok=True)

    print(f"Ablation A (naive answer prompt): {len(records)} samples")
    print(f"Run dir: {run_dir}")
    print("Reuses G2's final write-phase snapshot for all samples; query_hypothesis +")
    print("retrieval unchanged; only the final answer-generation prompt is swapped.")
    ...
    def _run_one(rec_idx):
        ...
        if not trace_path.exists():
            print(f"  [SKIP] abs_idx={abs_idx}: no group2 trace found at {trace_path}", flush=True)
            return None
        ...
        engine.naive_answer_prompt = True
