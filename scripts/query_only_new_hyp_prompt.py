#!/usr/bin/env python3
"""
Ablation G: new QUERY_HYPOTHESIS_PROMPT (state-change framing) on d215489 write-phase snapshots.

Loads G2's final write-phase state (profile_snapshot from
runs/d215489/e2ev3_400/{idx:04d}/trace.json) for all 400 samples,
then runs the query phase with `# no_query_hypothesis: use new prompt (default=False)` — retrieval
...
Run from /mnt/laq:
  python -m RECAST.scripts.query_only_no_query_hyp --run-name ablation_g --workers 8
"""
...
def parse_args():
    ...
    p.add_argument("--run-name", default="ablation_g")
...
def main():
    ...
    def _run_one(rec_idx):
        ...
        # no_query_hypothesis left False (default) — uses new QUERY_HYPOTHESIS_PROMPT
