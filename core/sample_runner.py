...
class SampleRunnerMixin:
    ...
    def run_sample(
        ...
        *,
        ...
        inner_workers = max(1, int(os.environ.get("RECAST_INNER_WORKERS", "16")))
        ...
        if indices:
            with ThreadPoolExecutor(max_workers=min(len(indices), inner_workers)) as ex:
                ...
                for fut in futures:
                    ...
                    try:
                        ...
                        print(f"[API ERROR] prescan future failed for session {idx} — will fall back to full extraction: {exc}", flush=True)
                        prescan[idx] = None
        ...
        for label, query_text in probing_queries.items():
            ...
            query_log = self.answer_query_v2(query_label=label, query_text=str(query_text))
