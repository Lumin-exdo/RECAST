...
class InsufficientBalanceError(Exception):
    """Account balance exhausted (HTTP 402). Never worth retrying — every
    layer that catches this should re-raise immediately instead of sleeping
    and retrying, and the top-level runner should stop the whole process."""
...
def _is_insufficient_balance(exc: Exception) -> bool:
    ...
    if getattr(exc, "status_code", None) == 402:
        return True
    return "Insufficient Balance" in str(exc)
...
class LLMClient:
    def __init__(
        ...
        *,
        ...
        # timeout=120s prevents indefinite hang when proxy returns empty/no response
        self.client = openai_cls(api_key=api_key, base_url=base_url, timeout=120.0)
        ...
        for attempt in range(max_retries):
            try:
                ...
                if isinstance(resp, str):
                    ...
                    text = _normalize_response_text(resp.choices[0].message.content)
                ...
                if _is_insufficient_balance(exc):
                    # Never worth retrying — fail fast instead of burning the
                    # remaining attempts (and every layer above this) on sleeps.
    ...
    def call_json(
        ...
        *,
        ...
        if request_kwargs is None and "openrouter.ai" in self.base_url.lower():
            request_kwargs = {"response_format": {"type": "json_object"}}
        raw = self.call_text(
            messages,
            max_retries=max_retries,
            extra_meta=extra_meta,
            extra_request_kwargs=request_kwargs,
        )
        try:
            ...
            bad_cache_path = self._cache_path(messages)
            if bad_cache_path is not None and bad_cache_path.exists():
                try:
